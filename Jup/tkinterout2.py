...
class Application(tk.Tk):
 ...
 #  def new_window(self):
    ...
    def reset_widgets(self):
        ...
        self.another_tbox1 = tk.Button(self.frame_1,text="Save State",command=self.create_new_state)
        self.another_tbox1.grid(row=2,column=0)
    def show_little_buttons(self,event):
        text_button = 
        text_button.grid(row=self.row_counter)
        eqn_button.grid(row=self.row_counter+=1)
        img_button.grid(row=self.row_counter+=2)
        graph_button.grid(row=self.row_counter+=3)
