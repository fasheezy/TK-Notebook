...
class Application(tk.Tk):
    def __init__(self, *args,**kwargs):
     ...
     #   self.frame_2.bind("<Configure>", self.on_configure)
       ...
       # self.main_canvas.create_window((600,0),window=self.frame_2,anchor = "NE")
        ...
        #self.entry_boxes["New Entry"+str(self.row_counter)] = tk.Entry(self.frame_1,bg="gainsboro",fg="grey",width=20,font=("Times New Roman",17))
    ...
    def make_new_entry(self):
        ...
        self.entry_boxes["New Entry"+str(self.row_counter)] = tk.Entry(self.frame_1)
        self.entry_boxes["New Entry"+str(self.row_counter)].insert(0,self.prefix_placeholder)
        self.entry_boxes["New Entry"+str(self.row_counter)].grid(row=self.row_counter,column=1,pady=3,sticky="w")
        self.entry_boxes["New Entry"+str(self.row_counter)].bind("<FocusIn>",self.clear_placeholder)
        self.entry_boxes["New Entry"+str(self.row_counter)].bind("<FocusOut>",self.add_placeholder) 
        self.entry_boxes["New Entry"+str(self.row_counter)].bind("<Return>",self.set_labellers)
    ...
    def clear_placeholder(self, event):
        ...
        if self.entry_boxes["New Entry"+str(self.row_counter)].get() == self.prefix_placeholder:
            self.entry_boxes["New Entry"+str(self.row_counter)].delete(0, tk.END)
            self.entry_boxes["New Entry"+str(self.row_counter)].config(foreground='grey')

    def add_placeholder(self, event):
        """Add the placeholder text when the entry loses focus if it's empty."""
        if self.entry_boxes["New Entry"+str(self.row_counter)].get() == "":
            self.entry_boxes["New Entry"+str(self.row_counter)].insert(0, self.prefix_placeholder)
            self.entry_boxes["New Entry"+str(self.row_counter)].config(foreground='grey')

    def set_labellers(self,event):
        if self.entry_boxes["New Entry"+str(self.row_counter)].get()!= "":
            self.entry_boxes["New Entry"+str(self.row_counter)].grid_forget()
 #  def new_window(self):
